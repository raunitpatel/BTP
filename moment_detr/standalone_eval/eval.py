...
def eval_main():
    input_folder  = "/home/mohit/moment_detr/run_on_video/prediction_files_second_sem_part_2_summarization_with_internvl2"
    ground_truth_path = "/home/mohit/moment_detr/run_on_video/icq_highlight_release.jsonl"
    save_dir = "/home/mohit/moment_detr/standalone_eval/results/results_second_sem_part_2_summarization_with_internvl2"
    os.makedirs(save_dir, exist_ok=True)

    styles   = ["cartoon", "cinematic", "realistic", "scribble"]
    prompts  = ["prompt_0", "prompt_1", "prompt_2", "prompt_3", "prompt_4", "prompt_5", "prompt_6", "prompt_7", "prompt_8", "prompt_9"]   # run1, run2, run3, run4, run5, run6, run7, run8, run9, run10
    temps    = ["temp_0.2"]
    run_names = {"prompt_0": "run1", "prompt_1": "run2", "prompt_2": "run3", "prompt_3": "run4", "prompt_4": "run5",
                 "prompt_5": "run6", "prompt_6": "run7", "prompt_7": "run8", "prompt_8": "run9", "prompt_9": "run10"}

    print("Loading ground truth...")
    gt = load_jsonl(ground_truth_path)

    # results[style][run][temp] = {"MR-full-mAP": ..., "MR-full-R1@0.5": ...}
    results = {s: {r: {} for r in ["run1", "run2", "run3", "run4", "run5", "run6", "run7", "run8", "run9", "run10"]} for s in styles}

    for style in styles:
        for prompt in prompts:
            run = run_names[prompt]
            for temp in temps:
                pred_path = os.path.join(input_folder, style, prompt, temp, "pred_output.jsonl")
                if not os.path.exists(pred_path):
                    print(f"⚠️  Missing: {pred_path}")
                    continue

                print(f"Evaluating: {style} | {run} | {temp}")
                submission = load_jsonl(pred_path)
                eval_results = eval_submission(submission, gt, verbose=False)

                # Save per-file JSON
                save_name = f"eval_{style}_{run}_{temp}.json"
                save_path = os.path.join(save_dir, save_name)
                with open(save_path, "w") as f:
                    json.dump(eval_results, f, indent=4)

                # Store key metrics
                results[style][run][temp] = {
                    "MR-full-mAP":    eval_results["brief"]["MR-full-mAP"],
                    "MR-full-R1@0.5": eval_results["brief"]["MR-full-R1@0.5"],
                }
                print(f"  mAP={results[style][run][temp]['MR-full-mAP']:.2f}  R1@0.5={results[style][run][temp]['MR-full-R1@0.5']:.2f}")

    # ------------------------------------------------------------------ #
    #  Save summary JSON
    # ------------------------------------------------------------------ #
    summary_path = os.path.join(save_dir, "summary_all_results.json")
    with open(summary_path, "w") as f:
        json.dump(results, f, indent=4)
    print(f"\n✅ Summary saved to {summary_path}")

    # # ------------------------------------------------------------------ #
    # #  Tables  (style × run, averaged over temps)
    # # ------------------------------------------------------------------ #
    # import pandas as pd

    # for metric in ["MR-full-mAP", "MR-full-R1@0.5"]:
    #     rows = []
    #     for style in styles:
    #         row = {"Style": style}
    #         for run in ["run1", "run2", "run3"]:
    #             for temp in temps:
    #                 val = results[style][run].get(temp, {}).get(metric, float("nan"))
    #                 row[f"{run}_{temp}"] = val
    #         rows.append(row)

    #     df = pd.DataFrame(rows).set_index("Style")
    #     csv_path = os.path.join(save_dir, f"table_{metric.replace('/', '_').replace('@','_')}.csv")
    #     df.to_csv(csv_path)
    #     print(f"\n📊 Table ({metric}):\n{df.to_string()}")
    #     print(f"✅ Table saved to {csv_path}")

    # # ------------------------------------------------------------------ #
    # #  Bar plots  (styles compared across runs, one plot per temp)
    # # ------------------------------------------------------------------ #
    ...
    #             vals = [
    #                 results[style][run].get(temp, {}).get(metric, 0.0)
    #                 for style in styles
    #             ]
    #             bars = ax.bar(x + offsets[i], vals, width, label=run, color=colors[i], alpha=0.85)
    ...
    #         ax.set_title(f"{metric}  —  {temp}", fontsize=13)
    #         ax.legend(title="Run")
    #         ax.set_ylim(0, max(
    #             results[s][r].get(temp, {}).get(metric, 0) for s in styles for r in ["prompt_0", "prompt_1", "prompt_2", "prompt_3", "prompt_4", "prompt_5", "prompt_6", "prompt_7", "prompt_8", "prompt_9"]
    #         ) * 1.2 + 1)
    #         ax.grid(axis="y", linestyle="--", alpha=0.5)

    #         plot_name = f"barplot_{metric.replace('/', '_').replace('@','_')}_{temp}.png"
